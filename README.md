...
```
go to [http://0.0.0.0:3003](http://0.0.0.0:3003) or [http://localhost:3003](http://localhost:3003) in your browser
...
## Running the app
After you have installed all dependencies you can now run the app. Run `npm run server` to start a local server using `webpack-dev-server` which will watch, build (in-memory), and reload for you. The port will be displayed to you as `http://0.0.0.0:3003` (or if you prefer IPv6, if you're using `express` server, then it's `http://[::1]:3003/`).
...
The included `.vscode` automatically connects to the webpack development server on port `3003`.
...
* How do I start the app when I get `EACCES` and `EADDRINUSE` errors?
  * The `EADDRINUSE` error means the port `3003` is currently being used and `EACCES` is lack of permission for webpack to build files to `./dist/`
